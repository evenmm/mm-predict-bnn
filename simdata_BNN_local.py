...
PLOT_SAVEDIR = "./plots/"
PICKLE_DIR = "./binaries_and_pickles/"
...
N_patients = 300
N_patients_test = 15 #300
advi_iterations_BNN = 100 #100_000
advi_iterations_lin = 100 #100_000 # 600_000
N_samples = 10 #10_000
N_tuning = 10 #10_000
...
plt.savefig(PLOT_SAVEDIR+"all_measurements_"+name_BNN+"_.pdf", dpi=300)
#plt.show()
plt.close()
...
try: 
    ...
    with lin_model:
        if ADADELTA_lin:
            ...
            hist_ax.set_title("Negative ELBO track")
            hist_ax.set_yscale("log")
            plt.savefig(PLOT_SAVEDIR+"0_elbo_and_trace.pdf", dpi=300)
            #plt.show()
            plt.close()
            print("-------------------SAMPLING-------------------")
            # Use approx as starting point for NUTS: https://www.pymc.io/projects/examples/en/latest/variational_inference/GLM-hierarchical-advi-minibatch.html
...
def get_absolute_errors_and_posterior_medians(local_name, PLOT_SAVEDIR, local_idata, local_model_name):
    ...
    try:
        picklefile = open(PLOT_SAVEDIR+local_name+'all_posterior_medians', "rb")
        ...
        picklefile = open(PLOT_SAVEDIR+local_name+'all_posterior_medians', "wb")
...
for elem in all_absolute_errors_sorted_BNN:
    plt.plot(range(len(elem)), elem)
plt.savefig(PLOT_SAVEDIR+"all_absolute_errors_sorted_BNN.pdf", dpi=300)
plt.title("BNN")
...
for elem in all_absolute_errors_sorted_lin:
    plt.plot(range(len(elem)), elem)
plt.title("Linear")
plt.savefig(PLOT_SAVEDIR+"all_absolute_errors_sorted_lin.pdf", dpi=300)
#plt.show()
plt.close()
...
plt.savefig(PLOT_SAVEDIR+"BNN_abs_errors_xaxis.pdf", dpi=300)
#plt.show()

# Boxplot with labels
fig, ax = plt.subplots()
ax.boxplot(all_absolute_errors_sorted_BNN_no_nan[1:], positions=plotting_times+0.2, widths=0.3, patch_artist=True, showfliers=False, boxprops=dict(facecolor="lightgrey"))
ax.boxplot(all_absolute_errors_sorted_lin_no_nan[1:], positions=plotting_times-0.2, widths=0.3, patch_artist=True, showfliers=False, boxprops=dict(facecolor="lightblue"))
...
#plt.legend()
plt.savefig(PLOT_SAVEDIR+"BNN_abs_errors.pdf", dpi=300)
plt.show()
